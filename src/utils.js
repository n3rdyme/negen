...
function toKebabCase(name) {
  name = name.replace(/^[A-Z]+/, function(txt) {
    return txt[1].toLowerCase();
  });
  name = name.replace(/[A-Z]+/g, function(txt) {
    return "-" + txt[1].toLowerCase();
  });
  return name;
}
...
function toSnakeCase(name) {
  ...
  name = name.replace(/[A-Z]+/g, function(txt) {
    return "_" + txt[1].toLowerCase();
  });
  return name;
}
exports.toSnakeCase = toSnakeCase;
function toPascalCase(name) {
  name = name.replace(/_\w/g, function(txt) {
...
function replaceAll(target, search, replacement) {
  ...
  if (search === "__name__") {
    result = result.replace(/__PascalName__/gi, function(val) {
      return toPascalCase(replacement);
    });
    result = result.replace(/__camelName__/gi, function(val) {
