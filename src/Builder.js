...
var Builder = (function() {
  ...
  Builder.prototype.addBlueprints = function(rootPath) {
    ...
    blueprints.forEach(function(blueprint) {
      ...
      if (typeof _this.genStore[name] != "undefined")
        throw new Error(
          "Duplicated blueprint " + name + ". The blueprint already exists."
        );
      else _this.genStore[name] = blueprint;
    });
  };
  ...
  Builder.prototype.build = function(blueprintName, moduleFullName, destPath) {
    var moduleParts = moduleFullName.split(/[\/\\]/g).filter(function(name) {
      return name && name !== ".";
    });
    var moduleName = moduleParts[moduleParts.length - 1];
    var modulePath = path.join.apply(
      path,
      moduleParts.slice(0, moduleParts.length - 1)
    );
    moduleParts
      .filter(function(name) {
        return !utils_1.isComplaintName(name, Builder.nameConstrain);
      })
      .forEach(function(name) {
        throw new Error(
          "Invalid argument '" +
            name +
            "'. Only characters, numbers and underscore allowed."
        );
      });
    fsx.ensureDir(destPath, function(err) {
      if (err) {
        throw new Error(
          "Error creating destination directory: " + destPath + " "
        );
      }
    });
    ...
    src_files.forEach(function(filePath) {
      ...
      if (stats.isDirectory()) {
        var dest_dir = Builder.replacePath(
          filePath,
          src_root,
          dest,
          moduleName,
        ...
        var dest_file = Builder.replacePath(
          filePath,
          src_root,
          dest,
          moduleName,
        ...
        var newContent = utils_1.replaceAll(
          content,
          Builder.replaceName,
          moduleName
        );
        dest_file = utils_1.replaceAll(
          dest_file,
          Builder.replaceName,
          moduleName
        );
    ...
    if (blueprint.script && fsx.existsSync(blueprint.script)) {
      ...
      child_process.execFileSync(
        execFilePath,
        [path.resolve(blueprint.script), moduleName, JSON.stringify(jsonArgs)],
        { stdio: "inherit" }
      );
    }
  ...
  Builder.findBlueprints = function(rootPath) {
    ...
    if (names.length == 0) throw Error("No Blueprint found in " + rootPath);
    names.forEach(function(blueprintName) {
      var name = blueprintName;
      var blueprintPath = path.join(rootPath, blueprintName);
      var scriptFile = fsx.existsSync(blueprintPath + ".js")
        ? blueprintPath + ".js"
        : fsx.existsSync(blueprintPath + ".ts")
        ? blueprintPath + ".ts"
        : null;
  ...
  Builder.replacePath = function(
    ...
    destpath = destpath.replace(/__path__/gi, function(val) {
      return modulePath;
    });
    destpath = destpath.replace(/__PascalPath__/gi, function(val) {
      return util.toPascalCase(modulePath);
    });
    destpath = destpath.replace(/__camelPath__/gi, function(val) {
      return util.toCamelCase(modulePath);
    });
