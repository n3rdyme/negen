...
var ARGS = yargs
  ...
  .command(
    ...
    function(yergs) {
      return yergs.pkgConf("negen", process.cwd());
    },
    function(argv) {
      command = true;
  ...
  .example(
    name + " generate ng_component my_new_component",
    'generates a new my-new-component using the "ng_component" Blueprint.'
  )
  .example(
    name + " list -b ./src/blueprints",
    "list all the blueprint within the './src/blueprints' directory."
  )
  .epilog(
    chalk.green("Config file:\n") +
      "package.json is used as config file, All [" +
      chalk.cyan("options") +
      ...
      chalk.blue("negen") +
      '":{"' +
      chalk.cyan("blueprints_root") +
      '":"./tools/blueprints/","' +
      chalk.cyan("modules_root") +
      '":"./src/modules/"}}'
  )
  .help()
  .wrap(null).argv;
if (ARGS._.length == 0) {
  ...
  console.error(chalk.red("negen <command:'" + ARGS._[0] + "'>, not found."));
  console.log("Use 'negen --help' to see available commands.");
}
